...
int MicroSoft_Joystick::Initialize(Property parameter)
{
	if( SetParameter(parameter) == API_ERROR ) {
        std::cout<<"## (API)MicroSoft_Joystick::Initialize 실패! ##"<<std::endl;
		return API_ERROR;
	}
    std::cout<<"## (API)MicroSoft_Joystick::Initialize 완료! ##"<<std::endl;
	return API_SUCCESS;
}
...
{
   // std::cout<<"## (API)MicroSoft_Joystick::GetJoystickData 시작! ##"<<std::endl;
	JOYINFOEX joyinfo;
	...
	if (joyGetPosEx(JOYSTICKID1 , &joyinfo) == JOYERR_NOERROR) {
       ...
       // std::cout<<"## (API)MicroSoft_Joystick::GetJoystickData 성공! ##"<<std::endl;
		return API_SUCCESS;
	}
	else {
